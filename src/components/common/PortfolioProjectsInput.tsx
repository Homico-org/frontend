    ...
    >
      ...
      {!compact && (
        <>
          ...
          <div className="absolute top-2.5 right-2.5 px-2.5 py-1 bg-gradient-to-r from-emerald-600/90 to-emerald-500/80 backdrop-blur-md rounded-md text-[10px] font-semibold text-white uppercase tracking-wider shadow-lg">
            After
          </div>
        </>
      )}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        {!isAddingProject && projects.length < maxProjects && (
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-medium transition-all duration-200 bg-emerald-500 text-white hover:bg-emerald-600 shadow-md shadow-emerald-500/20 hover:shadow-lg hover:shadow-emerald-500/25 active:scale-[0.98]"
          >
      ...
      {projects.length > 0 && !isAddingProject && (
        <div className="space-y-3">
          {projects.map((project, index) => (
            <div
              key={project.id}
              className="group relative rounded-2xl overflow-hidden border border-[var(--color-border)] bg-[var(--color-bg-secondary)] hover:border-emerald-500/30 hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-300"
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="p-2 rounded-lg text-[var(--color-text-tertiary)] hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-500/10 transition-colors"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {projects.length === 0 && !isAddingProject && (
        <button
          type="button"
          onClick={handleAddProject}
          className="w-full py-8 rounded-2xl border-2 border-dashed border-[var(--color-border)] hover:border-emerald-500/40 bg-gradient-to-br from-[var(--color-bg-secondary)]/80 to-[var(--color-bg-tertiary)]/30 transition-all duration-300 group"
        >
          <div className="flex flex-col items-center gap-3">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500/15 to-teal-500/10 flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-emerald-500/20 border border-emerald-500/20">
              <svg className="w-7 h-7 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v16m8-8H4" />
              </svg>
            </div>
            <div className="text-center">
              <p className="font-semibold text-[var(--color-text-secondary)] group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
      ...
      {isAddingProject && (
        <div className="rounded-2xl border border-[var(--color-border)] overflow-hidden bg-[var(--color-bg-secondary)] shadow-xl">
          {/* Form Header */}
          <div className="px-5 py-4 border-b border-[var(--color-border-subtle)] bg-gradient-to-r from-emerald-500/5 to-transparent flex items-center justify-between">
          ...
          <div className="p-5 space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-4 py-3 rounded-xl border-2 bg-[var(--color-bg-primary)] text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-emerald-500/50 transition-all resize-none"
                style={{ borderColor: 'var(--color-border)' }}
              />
            </div>
                  ...
                  {currentProject.images.map((url, idx) => (
                    <div key={idx} className="relative aspect-square rounded-lg overflow-hidden group">
                      ...
                      {idx === 0 && currentProject.beforeAfterPairs.length === 0 && (
                        <div className="absolute bottom-1 left-1 px-1.5 py-0.5 bg-emerald-500 rounded text-[8px] font-bold text-white">
                          Cover
                        </div>
                      )}
                    </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <button
                    ...
                    className="w-full p-4 rounded-xl border-2 border-dashed border-[var(--color-border)] hover:border-emerald-500/40 bg-[var(--color-bg-tertiary)]/30 transition-all duration-200 flex flex-col items-center gap-2 text-[var(--color-text-secondary)] hover:text-emerald-600 dark:hover:text-emerald-400 group"
                  >
