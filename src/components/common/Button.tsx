...
export default function Button({
  ...
  const content = (
    <>
      ...
      {!loading && icon && iconPosition === "left" && (
        <span
          className={`${iconSizes[size]} flex-shrink-0 transition-transform duration-300 group-hover:-translate-y-0.5`}
        >
          {icon}
        </span>
      )}
  ...
  const combinedClasses =
    `${baseClasses} ${sizeClasses[size]} ${fullWidth ? "w-full" : ""} ${className}`.trim();
