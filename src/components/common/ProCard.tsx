    ...
    return (
      <Link href={`/professionals/${profile._id}`} className="group block touch-manipulation">
        ...
        >
          ...
          {/* Portfolio Image Row - Fixed Height */}
          {hasPortfolio && (
            <div
              ref={sliderRef}
            ...
            >
              {portfolioImages.slice(0, 6).map((img, idx) => (
                ...
                >
                  <img
                    src={img.includes('?') ? `${img}&fit=crop` : `${img}?fit=crop`}
                    alt={`Portfolio ${idx + 1}`}
              ...
              {portfolioImages.length > 6 && (
                <div
                  className="flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-lg flex items-center justify-center"
      ...
      <Link href={`/professionals/${profile._id}`} className="group block h-full">
        ...
        >
          ...
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            {/* Status badge - refined positioning */}
            <div className="absolute top-3 right-3 z-10">
              <StatusBadge status={status} variant="minimal" />
            </div>

            {/* Name overlay with better typography */}
            <div className="absolute bottom-0 left-0 right-0 p-3">
              <h3 className="font-semibold text-[15px] text-white truncate leading-tight tracking-tight">
                {displayName}
              </h3>
          ...
          {/* Stats Bar - refined with better visual hierarchy */}
          <div
            className="grid grid-cols-4 gap-0 py-2.5 px-1"
            style={{ backgroundColor: 'var(--color-bg-tertiary)' }}
          >
            {/* Rating */}
            <div className="flex flex-col items-center text-center">
              ...
              <span className="text-[8px] leading-tight mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
                რეიტინგი
              </span>
            </div>
          ...
          {/* Portfolio Preview - Fixed Height Row */}
          {hasPortfolio && (
            <div
