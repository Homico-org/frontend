    ...
    <div className="group relative">
      <Link href={`/jobs/${job.id}`} className="block">
        <div className="bg-[var(--hm-bg-elevated)] rounded-lg sm:rounded-xl overflow-hidden border border-[var(--hm-border)] hover:border-[var(--hm-border-strong)] transition-all duration-200 sm:hover:shadow-md">
          ...
          <div className="flex gap-2 sm:gap-3 p-2 sm:p-3">
            ...
            <div className="relative w-12 h-12 sm:w-16 sm:h-16 rounded-md sm:rounded-lg overflow-hidden bg-[var(--hm-bg-tertiary)] flex-shrink-0">
              {firstImage && !imageError ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#F06B43] to-[#A92B08]">
            ...
            <div className="flex-1 min-w-0">
              {/* Title */}
              <h3 className="font-semibold text-xs sm:text-sm text-[var(--hm-fg-primary)] leading-tight line-clamp-1 mb-0.5 sm:mb-1">
                {job.title}
              </h3>

              {/* Category - Location hidden on mobile */}
              <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-[var(--hm-fg-muted)] mb-1 sm:mb-1.5">
              ...
              {/* Budget */}
              <div className="text-xs sm:text-sm font-semibold text-[var(--hm-brand-500)]">
                {formattedBudget}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between px-2 py-1.5 sm:px-3 sm:py-2 border-t border-[var(--hm-border-subtle)] bg-neutral-50/30">
            <div className="flex items-center gap-2 sm:gap-4">
              {/* Proposals */}
              <div className={`flex items-center gap-1 sm:gap-1.5 ${proposalCount > 0 ? 'text-[var(--hm-brand-500)]' : 'text-neutral-400'}`}>
              ...
              {/* Views */}
              <div className="flex items-center gap-1 sm:gap-1.5 text-[var(--hm-fg-muted)]">
            ...
            {/* Time */}
            <span className="text-[10px] sm:text-xs text-[var(--hm-fg-muted)]">
              {timeAgo}
            </span>
          </div>
        </div>
      ...
      {proposalCount > 0 && (
        <div className="absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 min-w-[16px] sm:min-w-[20px] h-4 sm:h-5 px-1 sm:px-1.5 rounded-full bg-[var(--hm-brand-500)] text-white text-[10px] sm:text-xs font-semibold flex items-center justify-center shadow-sm">
