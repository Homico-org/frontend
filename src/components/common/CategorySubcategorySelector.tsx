...
export default function CategorySubcategorySelector({
  ...
  const { categories: CATEGORIES, loading: categoriesLoading } =
    useCategories();

  // Support both old single category and new multiple categories API
  const selectedCategories: string[] =
    selectedCategoriesProp || (selectedCategory ? [selectedCategory] : []);
    ...
    try {
      ...
      return CATEGORIES.map((category) => {
        ...
        const categoryMatches =
          regex.test(category.name) || regex.test(category.nameKa);
  ...
  const addCustomSpecialty = () => {
    ...
    if (
      ...
      onCustomSpecialtiesChange([...selectedSubcategories, trimmed]);
  ...
  const removeCustomSpecialty = (specialty: string) => {
    if (onCustomSpecialtiesChange) {
      onCustomSpecialtiesChange(
        selectedSubcategories.filter((s) => s !== specialty)
      );
    }
  };
  ...
  const totalSelected = selectedSubcategories.length;
              ...
              onClick={() => {
                ...
                if (!isExpanded && !isSelected) {
                  setExpandedCategories(
                    (prev) => new Set([...prev, category.key])
                  );
                }
              }}
              onKeyDown={(e) => {
                  ...
                  {category.subcategories.map((sub, subIndex) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleSubcategoryToggle(category.key, sub.key)
                        }
