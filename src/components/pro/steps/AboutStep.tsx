...
export default function AboutStep({
  ...
  const handleGenerateBio = async () => {
    ...
    setAiError('');
    try {
      const generatedBio = await aiService.generateBio(formData.bio.trim(), locale, country);
      if (generatedBio) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                variant="filled"
                inputSize="default"
                placeholder={`${countries[country as CountryCode]?.phonePrefix ?? '+995'} ${countries[country as CountryCode]?.placeholder ?? '5XX XXX XXX'}`}
                leftIcon={
