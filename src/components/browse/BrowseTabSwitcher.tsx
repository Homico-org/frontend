...
export default function BrowseTabSwitcher({
  ...
  return (
    <div className="w-full grid grid-cols-2 gap-2 p-1 rounded-xl sm:rounded-2xl" style={{ backgroundColor: 'var(--color-bg-tertiary)' }}>
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`
              relative flex items-center justify-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3.5 rounded-lg sm:rounded-xl
          ...
          >
            ...
            {/* Text content */}
            <span
              className={`text-[13px] sm:text-base transition-colors duration-300 ${
                isActive
            ...
            {isActive && (
              <div
                className="absolute bottom-1 sm:bottom-1.5 left-1/2 -translate-x-1/2 w-6 sm:w-8 h-0.5 sm:h-1 rounded-full bg-[var(--color-accent)]"
              />
            )}
          </button>
        );
