...
export const PRO_STEPS: StepConfig[] = [
  {
    id: "account",
    title: { en: "Create Account", ka: "ანგარიშის შექმნა" },
    subtitle: { en: "Your basic information", ka: "ძირითადი ინფორმაცია" },
  },
  {
    id: "category",
    ...
    subtitle: {
      ...
      ka: "რა სერვისებს თავაზობთ კლიენტებს?",
    },
  },
  {
    id: "services",
    title: { en: "Portfolio", ka: "პორტფოლიო" },
    subtitle: { en: "Showcase your expertise", ka: "აჩვენე შენი გამოცდილება" },
  },
  {
    id: "review",
    title: { en: "Review", ka: "გადახედვა" },
    subtitle: { en: "Confirm your details", ka: "დაადასტურე შენი მონაცემები" },
  },
];
...
export function useRegistration(options?: UseRegistrationOptions): UseRegistrationReturn {
  ...
  const [clientCustomAvatar, setClientCustomAvatar] = useState<string | null>(
    null,
  );
  ...
  const [verificationChannel, setVerificationChannel] =
    useState<VerificationChannel>("sms");
  ...
  const handleClientAvatarUpload = useCallback(
  ...
  const sendOtp = useCallback(
    async (channel: VerificationChannel = verificationChannel) => {
  ...
  const handleProjectVideoUpload = useCallback(
    (projectId: string, files: FileList | null) => {
      if (!files || files.length === 0) return;
  ...
  const removeProjectVideo = useCallback(
    (projectId: string, videoIndex: number) => {
      setPortfolioProjects((prev) =>
        prev.map((p) => {
          if (p.id === projectId) {
            return {
              ...p,
              videos: p.videos.filter((_, i) => i !== videoIndex),
            };
          }
          return p;
        }),
  ...
  const handleBeforeAfterUpload = useCallback(
    (projectId: string, type: "before" | "after", file: File) => {
      const reader = new FileReader();
      reader.onload = (e) => {
        const base64 = e.target?.result as string;
        setPortfolioProjects((prev) =>
  ...
  const removeBeforeAfterPair = useCallback(
    (projectId: string, pairId: string) => {
      setPortfolioProjects((prev) =>
        prev.map((p) => {
          if (p.id === projectId) {
            return {
              ...p,
              beforeAfterPairs: p.beforeAfterPairs.filter(
                (pair) => pair.id !== pairId,
              ),
            };
          }
  ...
  const canProceedFromAccount = useCallback((): boolean => {
    ...
    const passwordsMatch =
      userType === "client" ? formData.password === repeatPassword : true;
    return (
