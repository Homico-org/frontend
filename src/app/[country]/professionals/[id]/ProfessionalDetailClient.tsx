...
export default function ProfessionalDetailClient({
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (error.response?.status === 404) {
          setError(t("professional.profileNotFound"));
        } else {
          setError(error.message || t("professional.failedToLoadProfile"));
        }
      } finally {
        setIsLoading(false);
  ...
  const pricingMeta = useMemo(() => {
    ...
    if (normalized === "per_sqm" && (hasBase || hasMax)) {
      ...
      return {
        ...
        valueLabel: `${val}${sym}${t("timeUnits.perSqm")}`,
      };
    }
    if (normalized === "range" && hasBase && hasMax) {
      return {
        typeLabel: t("common.priceRange"),
        valueLabel: `${base}${sym} - ${max}${sym}`,
      };
    }
  ...
  const handleContact = () => {
    ...
    if (!user) {
      openLoginModal(`/messages?recipient=${profile?.id ?? ""}`);
      return;
    }
  ...
  // Nationwide / country labels - kept locally since they're not
  // city-table entries. Stored values like the legacy GE Georgian
  // string "საქართველოს მასშტაბით" still resolve correctly.
  const SPECIAL_LABELS: Record<string, Record<typeof locale, string>> = {
    nationwide: { en: "Nationwide", ka: "მთელი ქვეყანა", ru: "По всей стране" },
    countrywide: { en: "Nationwide", ka: "მთელი ქვეყანა", ru: "По всей стране" },
    "მთელი ქვეყანა": { en: "Nationwide", ka: "მთელი ქვეყანა", ru: "По всей стране" },
    "საქართველოს მასშტაბით": { en: "Nationwide", ka: "მთელი ქვეყანა", ru: "По всей стране" },
    georgia: { en: "Georgia", ka: "საქართველო", ru: "Грузия" },
    საქართველო: { en: "Georgia", ka: "საქართველო", ru: "Грузия" },
  };

  const translateCityLabel = (city: string) => {
    const special = SPECIAL_LABELS[city] || SPECIAL_LABELS[city.toLowerCase().trim()];
    if (special) return special[locale] || special.en;
    return translateCity(city, locale);
  };

  const totalCompletedJobs =
    (profile?.completedJobs || 0) + (profile?.externalCompletedJobs || 0);

  // Animated counters for the hero stat row. Each ramps from 0 -> target
  // once on first reveal (or instantly for prefers-reduced-motion).
  // Optional chaining + `|| 0` keeps the hook order stable across the
  // pre-load `if (isLoading) return` early-return below - same pattern
  // used in JobDetailClient.
  const profileViews = useCountUp(profile?.profileViewCount ?? 0);
  const reviewsCountAnim = useCountUp(reviews.length || profile?.totalReviews || 0);
  const jobsCountAnim = useCountUp(totalCompletedJobs);
      ...
      <div className="min-h-screen bg-[var(--hm-bg-page)]">
        ...
        <div className="py-20 text-center">
          <div className="max-w-md mx-auto px-4">
            ...
            <Button
              onClick={() => router.push(cl("/professionals"))}
              className="mt-6"
            >
              {t("common.goBack")}
            </Button>
  ...
  return (
    <div className="min-h-screen bg-[var(--hm-bg-page)]">
      ...
      {/* Floating back chip - appears once the user scrolls past 400px
          ...
          back affordance. Bottom-left so it stays clear of the AI
          chat / feedback FABs anchored bottom-right. */}
      <FloatingBack href={cl("/professionals")} />
      ...
      >
        <div className="bg-[var(--hm-bg-elevated)] rounded-2xl shadow-xl shadow-[var(--hm-n-900)]/[0.08] border border-[var(--hm-border-subtle)] p-4 sm:p-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              {canEdit && isEditingName ? (
                ...
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                  ...
                  {/* Response-time pill - sets expectation about
                      how fast this pro replies before the user
                      even messages. Hides when avgResponseTime is
                      unset / 0 / > 24h. */}
                  <ResponseTimeChip avgHours={profile.avgResponseTime} variant="block" />
                  {/* Away pill - pro has toggled themselves Away.
                      Sits next to response-time so the client sees
                      both signals at a glance before contacting. */}
                  {profile.status === 'away' && (
              ...
              {profile.serviceAreas?.length > 0 && (
                <div className="flex items-center gap-1 text-xs text-[var(--hm-fg-muted)]">
                  ...
                  <span>{translateCityLabel(profile.serviceAreas[0])}</span>
                </div>
              )}

              {/* Member-since line. Tells visitors whether this pro is
                  established on Homico or brand new - a fast trust
              ...
              {(() => {
                ...
                const label =
                  monthsAgo < 1
                    ? t("professional.memberSinceWeeks")
                    : monthsAgo < 12
                      ? t("professional.memberSinceMonths", { count: monthsAgo })
                      : t("professional.memberSinceYears", {
                          count: Math.floor(monthsAgo / 12),
                        });
                return (
              ...
              {editedPricingModel && (
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    leftIcon={<span className="text-sm">{currencySymbol({ country: profile?.country ?? 'GE' })}</span>}
                    placeholder="0"
                    className="flex-1"
                  />
                  {editedPricingModel === PricingModel.RANGE && (
          ...
          {/* Stats row. Integer stats use the count-up hook so they
              ramp from 0 to target on first reveal (matches the
              job-detail page hero treatment). Rating stays as the
              decimal-format toFixed value - count-up reads weird on
              fractional numbers. */}
          <div className="flex items-center justify-around mt-3 pt-3 border-t border-[var(--hm-border-subtle)]">
            <div className="text-center">
              <p className="text-base font-bold text-[var(--hm-fg-primary)] tabular-nums">
                {profileViews}
              </p>
            ...
            {profile.avgRating > 0 && (
              <div className="text-center">
                <p className="text-base font-bold text-[var(--hm-fg-primary)] flex items-center justify-center gap-1 tabular-nums">
            ...
            <div className="mt-4 pt-4 border-t border-[var(--hm-border-subtle)]">
              {phoneRevealed && profile.phone ? (
                <a
                  ...
                  onClick={() => trackEvent(AnalyticsEvent.PHONE_CLICK, { proId: profile.id, proName: profile.name })}
                                    ...
                                    {(() => {
                                      ...
                                      return Object.entries(byService).map(([svcKey, entries]) => {
                                        ...
                                        return (
                                          <div
                                            key={svcKey}
                                            className="rounded-xl p-3 transition-all duration-200 hover:-translate-y-[1px] hover:shadow-md"
                                            style={{
                                              backgroundColor: 'var(--hm-bg-elevated)',
                                              border: '1px solid var(--hm-border-subtle)',
                                              boxShadow:
                                                '0 1px 2px 0 rgba(15, 23, 42, 0.03), 0 2px 8px -1px rgba(15, 23, 42, 0.03)',
                                            }}
                                          >
                                            {(() => {
                                              // Range-aware price formatter: emits "X - Y{sym}" when both
                                              // priceMin/priceMax are set and meaningfully differ;
                                              // otherwise falls through to the legacy single-price display.
                                              // Symbol resolved from the entry's `currency` (preferred)
                                              // or the pro's marketplace country.
                                              const sym = currencySymbol({ country: profile?.country ?? 'GE' });
                                              const formatPrice = (e: { price: number; priceMin?: number; priceMax?: number; currency?: string }): string => {
                                                const s = e.currency ? currencySymbol({ currency: e.currency }) : sym;
                                                if (
                                                  ...
                                                  return `${e.priceMin} - ${e.priceMax}${s}`;
                                                }
                                                return `${e.price}${s}`;
                                              };
                                                              ...
                                                              {tiers.map((tier, i) => {
                                                                ...
                                                                return (
                                                                  <div key={i} className="flex items-center gap-1.5 text-[11px] px-2">
                                                                    ...
                                                                    <span className="mx-0.5" style={{ color: 'var(--hm-fg-muted)' }}>→</span>
                                                                    <span className="font-bold text-[var(--hm-success-500)]">{discounted}{sym}</span>
                                                                    <span className="text-[10px] opacity-40 line-through" style={{ color: 'var(--hm-fg-secondary)' }}>{entry.price}{sym}</span>
                                                                  </div>
                                                                );
                                                              })}
                                                            </div>
            ...
            <div className="space-y-8 sm:space-y-10">
              ...
              <section>
                ...
                <AboutTab
                  ...
                  isOwner={canEdit}
                  onSocialLinkClick={(channel) => {
                    // Only WhatsApp has a dedicated event today - other
                    // channels roll up into CONTACT_REVEAL with label=channel
                    // so they're still countable in the admin dashboard.
                    if (channel === 'whatsapp') {
      ...
      {/* ========== STICKY BOTTOM CTA - MOBILE ==========
          Repackaged from the previous "floating button with margin" UI
          into a full-bleed glass bar pinned to the bottom of the
          viewport. Matches the same pattern as the job-detail page so
          the marketplace has one consistent mobile CTA style. Glass
      ...
      >
        <div className="bg-[var(--hm-bg-elevated)]/95 backdrop-blur-xl border-t border-[var(--hm-border)] shadow-[0_-8px_32px_rgba(0,0,0,0.12)] px-3 sm:px-4 py-3 space-y-2">
        {/* Book service - mobile. Mirrors the desktop sidebar's
            booking CTA so a mobile visitor sees the same primary
            action. Same visibility rules as desktop: feature flag
            on, not the pro themselves, and the pro has at least
            one active priced service. Without this, the entire
            mobile audience had no way into the booking flow short
            of switching to a wider window. */}
        {features.bookings && !canEdit &&
          (profile?.servicePricing?.filter(
            (s) => s.isActive && s.price > 0,
          ).length ?? 0) > 0 && (
        ...
        {myOpenJobsLoaded && myMatchingOpenJobs.length > 0 && (
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={() => setShowInviteToJobModal(true)}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-[var(--hm-fg-secondary)] font-semibold text-sm bg-[var(--hm-bg-tertiary)] border border-[var(--hm-border-subtle)] hover:border-[var(--hm-border-strong)] transition-colors"
          >
                ...
                {videos.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    {videos.map((vid, idx) => (
                      <div
                        key={idx}
                        className="relative aspect-video rounded-xl overflow-hidden group ring-1 ring-[var(--hm-border-subtle)] bg-neutral-900"
                      >
                        <video
                          src={storage.getFileUrl(vid)}
                          className="w-full h-full object-cover"
                ...
                {beforeAfterPairs.map((pair, idx) => (
                  ...
                  >
                    <div className="flex-1 space-y-1.5">
                      ...
                      <div className="relative aspect-[4/3] rounded-lg overflow-hidden ring-1 ring-[var(--hm-border-subtle)]">
                        <Image
