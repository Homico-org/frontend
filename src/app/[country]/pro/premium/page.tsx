...
export default function PremiumPlansPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--hm-bg-page)]">
      ...
      <main className="mx-auto flex min-h-[calc(100vh-var(--hm-header-h,72px))] max-w-md flex-col justify-center px-5 py-6">
        {/* ── Hero (compact) ────────────────────────────────── */}
        <div className="text-center">
          <p className="mb-3 font-mono text-[10px] uppercase tracking-[0.24em] text-[var(--hm-brand-500)]">
            Homico Premium
          </p>
          <h1 className="text-[26px] font-semibold leading-[1.12] tracking-[-0.03em] text-[var(--hm-fg-primary)] sm:text-[32px]">
            {pick({
          ...
          <p className="mx-auto mt-3 max-w-sm text-[14px] leading-relaxed text-[var(--hm-fg-muted)]">
            {pick({
              en: "Top of search, on the homepage - right where clients choose.",
              ka: "ძიების თავში და მთავარ გვერდზე. იქ, სადაც კლიენტი ირჩევს.",
              ru: "Вверху поиска и на главной - там, где клиент выбирает.",
            })}
          </p>
        </div>

        {/* ── Plan card (light) ─────────────────────────────── */}
        <div className="mt-5 overflow-hidden rounded-3xl border border-[var(--hm-border-subtle)] bg-[var(--hm-bg-elevated)] shadow-[var(--hm-shadow-md)]">
          <div className="p-5 sm:p-6">
            ...
            {isActive ? (
              <p className="mt-4 text-[15px] text-[var(--hm-fg-muted)]">
                {premiumExpiresAt
              ...
              <div className="mt-4 flex items-baseline gap-1.5">
                <span className="text-[46px] font-semibold leading-none tabular-nums tracking-[-0.04em] text-[var(--hm-fg-primary)]">
                  {currency}
                  {price}
                </span>
            ...
            {isActive ? (
              refundable ? (
                <button
                  ...
                  className="mt-3 w-full text-center text-[12px] text-[var(--hm-fg-muted)] underline-offset-4 transition-colors hover:text-[var(--hm-error-500)] hover:underline"
                >
                  {pick({
                ...
                <p className="mt-3 text-center text-[12px] text-[var(--hm-fg-muted)]">
                  {pick({
          ...
          {/* What's included */}
          <div className="border-t border-[var(--hm-border-subtle)] bg-[var(--hm-bg-page)]/40 p-5 sm:p-6">
            <p className="mb-3.5 font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--hm-fg-subtle)]">
              {t("premium.includes")}
            </p>
        ...
        {/* ── Trust (compact, one line) ─────────────────────── */}
        <div className="mt-5 flex items-center justify-center gap-5 text-[12px] text-[var(--hm-fg-muted)]">
          <span>
            <span className="font-semibold text-[var(--hm-fg-primary)]">{VERIFIED_PROS}</span>{" "}
          ...
          <span className="flex items-center gap-1">
            ...
            {pick({ en: "rating", ka: "შეფასება", ru: "рейтинг" })}
          </span>
        </div>
      </main>
