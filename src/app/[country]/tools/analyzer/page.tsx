...
export default function AnalyzerPage() {
  ...
  const analyzeWithAI = useCallback(async (text: string) => {
    ...
    setError(null);
    try {
      const result = await aiService.analyzeEstimate(text, locale, country);
  ...
  const handleFileSelect = useCallback(async (selectedFile: File) => {
    ...
    try {
      ...
      const result = await aiService.analyzeEstimate(extractedText, locale, country);
      setAiAnalysis(result);
      setExpandedCategories(new Set());
    } catch (err: any) {
...
- Radiator installation (8 pcs) - price not specified`;
  ...
  const formatCurrency = (amount: number | null | undefined) =>
    amount != null ? amount.toLocaleString() + sym : '-';
      ...
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl">
          ...
          {!analysis && !aiAnalysis && !isAnalyzing ? (
            ...
            /* AI Analysis Results */
            <div className="space-y-4 tool-print-report">
                                ...
                                <div key={item.id} className="px-4 py-4 flex items-center justify-between gap-4">
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-3 mt-1 text-xs text-[var(--hm-fg-muted)]">
                                      {item.unitPrice > 0 && (
                                        <span>
                                          {item.quantity} × {item.unitPrice}{sym}
                                        </span>
                                      )}
                                      {item.marketAverage > 0 && (
                                        <span className="flex items-center gap-1">
                                          <span className="text-[var(--hm-fg-muted)]">{t('tools.analyzer.marketAvg')}:</span>
                                          <span>{item.marketAverage}{sym}</span>
                                        </span>
                                      )}
                                    </div>
                                  </div>
          ...
          <RelatedTools current="analyzer" className="mt-10" />
        </div>
      </div>
    </div>
  );
