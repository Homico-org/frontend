...
const STEPS: { id: Step; title: { en: string; ka: string }; icon: React.ReactNode }[] = [
  { id: 'category', title: { en: 'Specialty', ka: 'სპეციალობა' }, icon: <Briefcase className="w-4 h-4" /> },
  { id: 'about', title: { en: 'About You', ka: 'შენს შესახებ' }, icon: <FileText className="w-4 h-4" /> },
  { id: 'contact', title: { en: 'Contact', ka: 'კონტაქტი' }, icon: <MessageCircle className="w-4 h-4" /> },
  { id: 'review', title: { en: 'Review', ka: 'გადახედვა' }, icon: <CheckCircle2 className="w-4 h-4" /> },
];
...
export default function BecomeProPage() {
  ...
  const { user, isLoading: authLoading, isAuthenticated, login, updateUser } = useAuth();
  ...
  const handleCategoryToggle = (categoryKey: string) => {
    if (selectedCategories.includes(categoryKey)) {
      ...
      if (category) {
        ...
        setSelectedSubcategories(selectedSubcategories.filter(s => !categorySubKeys.includes(s)));
      }
  ...
  const handleSubcategoryToggle = (subcategoryKey: string) => {
    if (selectedSubcategories.includes(subcategoryKey)) {
      setSelectedSubcategories(selectedSubcategories.filter(s => s !== subcategoryKey));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/users/upgrade-to-pro`, {
        ...
        body: JSON.stringify({
          selectedCategories: [...selectedCategories, ...selectedSubcategories],
          bio,
    ...
    return (
      <div className="min-h-screen bg-[var(--color-bg-primary)] overflow-hidden">
            ...
            {[...Array(40)].map((_, i) => (
              <div
                ...
                style={{
                  ...
                  top: '-20px',
                  backgroundColor: ['#C4735B', '#E8A593', '#D98B74', '#F5DCD4', '#A85D47'][Math.floor(Math.random() * 5)],
        ...
        <div className="relative z-10 min-h-screen flex items-center justify-center px-4">
          <div className="max-w-md w-full text-center" style={{ animation: 'scale-in 0.5s ease-out' }}>
            ...
            <div className="space-y-3">
              <Link href="/browse/jobs">
                <Button variant="default" size="xl" className="w-full group" style={{ backgroundColor: TERRACOTTA.primary }}>
  ...
  if (currentStep === 'intro') {
    return (
      <div className="become-pro-container overflow-x-hidden">
        ...
        <main className={`relative z-10 pt-8 pb-32 transition-all duration-700 ease-out ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <section className="text-center mb-12">
                <div className="become-pro-hero-badge">
                  <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: TERRACOTTA.primary }}>
                ...
                <div className="grid grid-cols-3 gap-3 md:gap-4 mt-10">
                  {[
                    { icon: <Sparkles className="w-5 h-5" />, value: locale === 'ka' ? 'უფასო' : 'Free', label: locale === 'ka' ? 'რეგისტრაცია' : 'Registration' },
                    { icon: <Users className="w-5 h-5" />, value: '2,400+', label: locale === 'ka' ? 'კლიენტი' : 'Clients' },
                    { icon: <Shield className="w-5 h-5" />, value: '98%', label: locale === 'ka' ? 'კმაყოფილება' : 'Satisfaction' },
                  ].map((benefit, i) => (
                    <div key={i} className="become-pro-benefit-card" style={{ animationDelay: `${0.4 + i * 0.1}s` }}>
                      <div className="become-pro-benefit-icon">{benefit.icon}</div>
              ...
              <section className="bg-[var(--color-bg-elevated)] rounded-2xl p-6 border border-[var(--color-border-subtle)] mb-8">
                ...
                <div className="space-y-3">
                  {STEPS.map((step, i) => (
                    <div key={step.id} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--color-bg-tertiary)]">
                      <div
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className="flex-1">
                  <p className="text-xs text-[var(--color-text-tertiary)] uppercase tracking-wider">
                    {locale === 'ka' ? 'ნაბიჯი' : 'Step'} {getCurrentStepIndex() + 1} / {STEPS.length}
                  </p>
                  <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">
                    {locale === 'ka' ? STEPS[getCurrentStepIndex()].title.ka : STEPS[getCurrentStepIndex()].title.en}
                  </h2>
                </div>
              </div>
              ...
              <div className="flex gap-1.5">
                {STEPS.map((step, i) => (
                  <div
                    ...
                    style={{
                      backgroundColor: i <= getCurrentStepIndex() ? TERRACOTTA.primary : 'var(--color-border-subtle)',
                    }}
                  />
                ))}
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-[var(--color-bg-elevated)] rounded-2xl p-4 sm:p-6 border border-[var(--color-border-subtle)]">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        <Briefcase className="w-5 h-5" style={{ color: TERRACOTTA.primary }} />
                        <h3 className="font-semibold text-[var(--color-text-primary)]">
                          {locale === 'ka' ? 'აირჩიე კატეგორია' : 'Select Category'}
                          <span style={{ color: TERRACOTTA.primary }} className="ml-1">*</span>
                        </h3>
                      </div>
                      {validation.category && (
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              ...
                              style={{ backgroundColor: isSelected ? TERRACOTTA.primary : 'var(--color-bg-elevated)' }}
                            >
                              {categoryIcons[category.key] || <Briefcase className="w-5 h-5" />}
                            </div>
                  ...
                  {selectedCategories.length > 0 && availableSubcategories.length > 0 && (
                    <div className="bg-[var(--color-bg-elevated)] rounded-2xl p-4 sm:p-6 border border-[var(--color-border-subtle)]">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-[var(--color-text-primary)]">
                          {locale === 'ka' ? 'აირჩიე უნარები' : 'Select Skills'}
                          <span style={{ color: TERRACOTTA.primary }} className="ml-1">*</span>
                        </h3>
                        {selectedSubcategories.length > 0 && (
                          <span className="text-xs font-medium" style={{ color: TERRACOTTA.primary }}>
                        ...
                        {availableSubcategories.map((subcategory) => {
                          ...
                          return (
                            <button
                              ...
                              style={isSelected ? { backgroundColor: TERRACOTTA.primary } : {}}
                            >
                              {isSelected && (
                                <CheckCircle2 className="w-3.5 h-3.5" />
                              )}
                  ...
                  {selectedCategories.length > 0 && (
                    <div className="bg-[var(--color-bg-elevated)] rounded-2xl p-4 sm:p-6 border border-[var(--color-border-subtle)]">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="font-semibold text-[var(--color-text-primary)]">
                          {locale === 'ka' ? 'დამატებითი სერვისები' : 'Custom Services'}
                        ...
                        <div className="space-y-2 mb-4">
                          {customServices.map((service, index) => (
                            <div
                              ...
                              style={{ backgroundColor: `${TERRACOTTA.primary}10`, border: `1px solid ${TERRACOTTA.primary}20` }}
                            >
                              <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: TERRACOTTA.primary }} />
                              <span className="flex-1 text-sm text-[var(--color-text-primary)]">{service}</span>
                              <button
                                type="button"
              ...
              {currentStep === 'about' && (
                <div className="space-y-6">
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        <Camera className="w-5 h-5" style={{ color: TERRACOTTA.primary }} />
                        <h3 className="font-semibold text-[var(--color-text-primary)]">
                      ...
                      {validation.avatar ? (
                        ...
                        <span className="flex items-center gap-1.5 text-xs font-medium bg-[#C4735B]/10 px-2.5 py-1 rounded-full" style={{ color: TERRACOTTA.primary }}>
                      ...
                      {!avatarPreview ? (
                        ...
                        >
                          <div className="text-center">
                            <Camera className="w-8 h-8 mx-auto mb-1 transition-colors" style={{ color: `${TERRACOTTA.primary}60` }} />
                            <span className="text-xs font-medium" style={{ color: `${TERRACOTTA.primary}60` }}>
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        <Clock className="w-5 h-5" style={{ color: TERRACOTTA.primary }} />
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        <FileText className="w-5 h-5" style={{ color: TERRACOTTA.primary }} />
                        <span className="font-semibold text-[var(--color-text-primary)]">
                    ...
                    <Textarea
                      ...
                      placeholder={locale === 'ka' ? 'მოგვიყევი შენი გამოცდილების შესახებ...' : 'Tell us about your experience...'}
                    />
                    <div className="flex justify-between items-center mt-2">
                      <p className="text-xs text-[var(--color-text-muted)]">
                        {locale === 'ka' ? 'მინიმუმ 20 სიმბოლო' : 'Minimum 20 characters'}
                      </p>
                      <span className={`text-xs font-medium ${bio.length >= 20 ? 'text-emerald-600' : bio.length > 0 ? 'text-amber-600' : 'text-[var(--color-text-muted)]'}`}>
                        {bio.length}/500
                      </span>
                    </div>
                  </div>
              ...
              {currentStep === 'contact' && (
                <div className="space-y-6">
                  <div className="bg-[var(--color-bg-elevated)] rounded-2xl p-4 sm:p-6 border border-[var(--color-border-subtle)]">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        <MessageCircle className="w-5 h-5" style={{ color: TERRACOTTA.primary }} />
                        <h3 className="font-semibold text-[var(--color-text-primary)]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1.5">
                          WhatsApp <span style={{ color: TERRACOTTA.primary }}>*</span>
                        </label>
                        <Input
                          type="tel"
                          value={whatsapp}
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-[var(--color-bg-elevated)] rounded-2xl p-4 sm:p-6 border border-[var(--color-border-subtle)]">
                    <h3 className="font-semibold text-[var(--color-text-primary)] mb-6">
                      {locale === 'ka' ? 'პროფილის მიმოხილვა' : 'Profile Summary'}
                    </h3>
                      ...
                      <div>
                        ...
                        {yearsExperience && (
                          <p className="text-sm text-[var(--color-text-secondary)]">
                            {yearsExperience} {locale === 'ka' ? 'წლის გამოცდილება' : 'years experience'}
                          </p>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedSubcategories.map((subKey) => {
                            const subcategory = availableSubcategories.find(s => s.key === subKey);
                            return (
                              <span
                                key={subKey}
          ...
          <div className="flex items-center gap-3">
            ...
            {currentStep === 'review' ? (
              ...
              <Button
                ...
                size="lg"
                className="flex-1 group"
                style={{ backgroundColor: canProceed() ? TERRACOTTA.primary : undefined }}
              >
