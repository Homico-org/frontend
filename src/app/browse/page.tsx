...
function BrowseContent() {
  ...
  const showProfessionals =
    user?.role === "client" || (user?.role === "pro" && isClientMode);
  ...
  const [searchQuery, setSearchQuery] = useState(
    getInitialFilter("q", "browse_searchQuery", "") || ""
  );
  const [selectedCategory, setSelectedCategory] = useState<string | null>(
    getInitialFilter("category", "browse_category")
  );
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(
    getInitialFilter("subcategory", "browse_subcategory")
  );
  const [priceRange, setPriceRange] = useState([0, 1000]);
  const [minRating, setMinRating] = useState(0);
  const [sortBy, setSortBy] = useState(
    getInitialFilter("sort", "browse_sortBy", "recommended") || "recommended"
  );
  ...
  const [activeProTab, setActiveProTab] = useState<
    "jobs" | "proposals" | "active"
  >("jobs");
  const [proposalFilter, setProposalFilter] = useState<
    "all" | "pending" | "accepted" | "rejected"
  >("all");
  const [proViewMode, setProViewMode] = useState<"grid" | "list">(() =>
    storage.getProViewMode()
  );
  ...
  const userCategories = useMemo(
    () => JSON.parse(userCategoriesJson) as string[],
    [userCategoriesJson]
  );
  ...
  useEffect(() => {
    ...
    if (selectedCategory)
      localStorage.setItem("browse_category", selectedCategory);
    else localStorage.removeItem("browse_category");

    if (selectedSubcategory)
      localStorage.setItem("browse_subcategory", selectedSubcategory);
    ...
    const fetchCategories = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/pro-profiles/categories`
        );
  ...
  const fetchProfiles = useCallback(
    async (pageNum: number, append: boolean = false) => {
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/pro-profiles?${params}`
        );
        if (response.ok) {
          const result = await response.json();
          if (append) {
  ...
  const handleProLike = async (profileId: string) => {
    ...
    const currentState = likeStates[profileId] || {
      isLiked: false,
      likeCount: 0,
    };
        ...
        if (response.ok) {
          ...
          if (selectedBudgetType !== "all" && Array.isArray(jobsData)) {
            jobsData = jobsData.filter(
              (job: Job) => job.budgetType === selectedBudgetType
            );
          }
  ...
  const getTimeAgo = (date: string) => {
    ...
    if (seconds < 3600)
      return `${Math.floor(seconds / 60)}${t("jobs.minutesAgo")}`;
    if (seconds < 86400)
      return `${Math.floor(seconds / 3600)}${t("jobs.hoursAgo")}`;
  ...
  const activeJobs = useMemo(() => {
    return proposals.filter(
      (p) => p.status === "accepted" && p.jobId?.status === "in_progress"
    );
  }, [proposals]);
                  ...
                  {[
                    {
                      key: "jobs",
                      label: "სამუშაოები",
                      count: totalJobsCount || jobs.length,
                    },
                    {
                      key: "proposals",
                      label: "წინადადებები",
                      count: proposalStats.pending,
                    },
                    {
                      key: "active",
                      label: "აქტიური",
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="flex gap-2 sm:gap-3">
                  <div className="relative flex-1">
                    <svg
                      className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4"
                ...
                {isLoadingProposals ? (
                  <div className="space-y-3">
                    {[...Array(5)].map((_, i) => (
                      ...
                      >
                        <div
                          className="h-5 rounded-lg w-2/3 mb-2"
                          style={{
                            backgroundColor: "var(--color-bg-tertiary)",
                          }}
                        />
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start sm:items-center gap-2 mb-1 flex-wrap">
                              <h3
                                className="font-medium group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors line-clamp-2 sm:truncate"
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <h3
                                className="font-medium line-clamp-2 sm:truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors"
                                style={{ color: "var(--color-text-primary)" }}
                              >
                                {proposal.jobId?.title}
                              </h3>
      ...
      {!isPro && (
        <div className="relative min-h-[calc(100vh-64px)]">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-5 py-4 sm:py-8 relative z-10">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                <div>
                  ...
                  >
                    {activeClientTab === "professionals"
                      ? t("browse.subtitle") ||
                        "აღმოაჩინე გამოცდილი პროფესიონალები შენი პროექტისთვის"
                      : locale === "ka"
                        ? "დაათვალიერე დასრულებული პროექტები და პორტფოლიოები"
                        : "Browse completed projects and portfolios"}
                  </p>
                </div>
            ...
            {/* Categories Section with integrated rating filter - shared between both tabs */}
            <div className="mb-3 sm:mb-5">
              <CategorySection
                ...
                showRatingFilter={activeClientTab === "professionals"}
              />
            </div>
              ...
              <>
                ...
                <div className="flex items-center gap-2 mb-4 sm:mb-5">
                  ...
                  <div className="relative flex-1">
                    ...
                    <input
                      type="text"
                      placeholder={
                        t("browse.searchPlaceholder") || "სპეციალისტის ძებნა..."
                      }
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full h-10 pl-10 pr-4 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/20 focus:border-[var(--color-accent)]/50 transition-all"
                      style={{
                        backgroundColor: "var(--color-bg-secondary)",
                        borderColor: "var(--color-border)",
                        color: "var(--color-text-primary)",
                  ...
                  {/* Sort & View Controls - Combined group */}
                  <div
                  ...
                  >
                    ...
                    {[
                      {
                        value: "recommended",
                        label: locale === "ka" ? "რეკომ." : "Top",
                      },
                      {
                        value: "rating",
                        label: locale === "ka" ? "რეიტ." : "Rating",
                      },
                      {
                        value: "newest",
                        label: locale === "ka" ? "ახალი" : "New",
                      },
                    ].map((option) => (
                      <button
                        key={option.value}
                        ...
                        className={`
                          ...
                          ${
                            sortBy === option.value
                              ? "bg-white dark:bg-dark-elevated shadow-sm text-[var(--color-text-primary)]"
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-md transition-all ${viewMode === "list" ? "bg-white dark:bg-dark-elevated shadow-sm" : ""}`}
                    >
                      <svg
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-md transition-all ${viewMode === "grid" ? "bg-white dark:bg-dark-elevated shadow-sm" : ""}`}
                    >
                      <svg
                        className="w-3.5 h-3.5"
                        style={{
                  ...
                  {(selectedCategory ||
                    ...
                    <button
                      onClick={clearFilters}
                      className="h-10 w-10 rounded-xl text-sm font-medium transition-all flex items-center justify-center flex-shrink-0 touch-manipulation bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 border border-red-200 dark:border-red-500/30"
                      title={t("browse.clearFilters") || "გასუფთავება"}
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
                ...
                <div className="flex items-center mb-3 sm:mb-4">
                  ...
                  >
                    ...
                    {t("browse.specialists") || "სპეციალისტი"}
                    {selectedCategory && (
                      <span
                        className="inline-block ml-2 px-2 py-0.5 rounded-md text-xs"
                        style={{
                    ...
                    {selectedSubcategory && (
                      <span
                        className="inline-block ml-1 px-2 py-0.5 rounded-md text-xs"
                        style={{
                          backgroundColor: "var(--color-bg-tertiary)",
                          color: "var(--color-text-secondary)",
                        }}
                      >
                        {(() => {
                          ...
                          return locale === "ka"
                            ? sub?.nameKa || selectedSubcategory
                      ...
                      >
                        <ProCard
                          profile={{
                            ...profile,
                            isLiked:
                              likeStates[profile._id]?.isLiked ??
                              profile.isLiked,
                            likeCount:
                              likeStates[profile._id]?.likeCount ??
                              profile.likeCount,
                          }}
