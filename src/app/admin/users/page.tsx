...
function AdminUsersPageContent() {
  ...
  // Deduplicate duplicate calls (e.g., React 18 StrictMode double-effect in dev)
  ...
  const fetchData = useCallback(
    async (showRefresh = false) => {
      ...
      try {
        // After first load, treat subsequent loads as "refresh" so the page doesn't look like a full refresh.
        ...
        toast.error(
          locale === "ka"
            ? "ვერ მოხერხდა მომხმარებლების ჩატვირთვა"
  ...
  const getRoleLabel = (role: string) =>
    getAdminRoleLabel(role, locale as "en" | "ka" | "ru");
  ...
  const handleVerificationAction = async (action: "approve" | "reject") => {
    ...
    try {
      await api.patch(
        `/admin/users/${showVerificationModal._id}/verification`,
        {
      ...
      toast.error(
        locale === "ka" ? "ვერ მოხერხდა განახლება" : "Failed to update"
      );
    } finally {
          ...
          {statCards.map((stat) => (
            <button
              ...
              style={{
                ...
                boxShadow:
                  stat.role === roleFilter
                    ? `0 0 0 3px ${stat.color}14`
                    : undefined,
              }}
              onClick={() => {
                setRoleFilter((prev) =>
                  prev === stat.role ? "all" : stat.role
                );
                setPage(1);
              }}
              type="button"
              ...
              >
                ...
                <div className="col-span-2 hidden lg:flex items-center gap-2">
                  {user.isSuspended ? (
                    ...
                    // Show verification status for pros/companies (fallback to Active if none)
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                <Avatar
                  src={showVerificationModal.avatar}
                  name={showVerificationModal.name}
                  size="lg"
                />
                <div>
                  ...
                  >
                    ...
                    {showVerificationModal.idDocumentUrl ? (
                      <a
                        ...
                        className="relative block aspect-video rounded-lg overflow-hidden bg-black/20 hover:opacity-80 transition-opacity"
                      >
                        <NextImage
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {!showVerificationModal.facebookUrl &&
                    ...
                    !showVerificationModal.websiteUrl && (
                      <p
                        className="col-span-2 text-sm py-4 text-center"
                        style={{ color: THEME.textDim }}
                      >
