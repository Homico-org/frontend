...
import {
  Building2, CheckCircle, Clock, MapPin, MessageSquare, Shield, Star, Trophy,
  ChevronLeft, ChevronRight, X, ExternalLink, Award, Users, Verified, BadgeCheck,
  ArrowRight, Briefcase, Calendar, Phone, Globe, Mail
} from 'lucide-react';
...
export default function CompanyProfilePage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/companies/${params.id}/public`);
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/reviews/company/${profile._id}`);
        if (response.ok) {
          const data = await response.json();
          setReviews(data);
        }
        ...
        <div className="py-20 text-center">
          <div className="max-w-md mx-auto px-4">
            ...
            <p className="text-[var(--color-text-secondary)] mb-8">
              {locale === 'ka' ? 'სამწუხაროდ, ეს კომპანია არ არსებობს' : 'Sorry, this company doesn\'t exist'}
            </p>
            <button
  ...
  const getCompanyTypeLabel = () => {
    switch (profile.companyType) {
      case 'construction': return locale === 'ka' ? 'სამშენებლო კომპანია' : 'Construction Company';
      case 'service_agency': return locale === 'ka' ? 'სერვისის სააგენტო' : 'Service Agency';
      case 'both': return locale === 'ka' ? 'სამშენებლო & სერვისი' : 'Construction & Service';
      default: return locale === 'ka' ? 'კომპანია' : 'Company';
    }
  };
    ...
    <div className="min-h-screen bg-[var(--color-bg-primary)]">
      ...
      <section className={`relative transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className={`relative ${profile.coverImage ? '-mt-20' : 'pt-6 sm:pt-8'} mb-8`}>
              ...
              <div className="p-6 sm:p-8">
                <div className="flex flex-col sm:flex-row gap-6 sm:gap-8">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Tagline or Type */}
                    <p className="text-lg sm:text-xl font-medium mb-4" style={{ color: ACCENT }}>
                    ...
                    <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-sm">
                      ...
                      {(profile.city || profile.serviceAreas.length > 0) && (
                        <div className="flex items-center gap-1.5 text-[var(--color-text-secondary)]">
                          ...
                          <span>{translateCity(profile.city || profile.serviceAreas[0])}</span>
                        </div>
                      )}
                      ...
                      {profile.teamSize && profile.teamSize > 0 && (
                        <div className="flex items-center gap-1.5 text-[var(--color-text-secondary)]">
                          ...
                          <span>{profile.teamSize} {locale === 'ka' ? 'თანამშრომელი' : 'employees'}</span>
                        </div>
                      )}
                      ...
                      {profile.completedProjects && profile.completedProjects > 0 && (
                        <div className="flex items-center gap-1.5 text-[var(--color-text-secondary)]">
                          ...
                          <span>{profile.completedProjects} {locale === 'ka' ? 'პროექტი' : 'projects'}</span>
                        </div>
                      )}
                      ...
                      {profile.foundedYear && (
                        <div className="flex items-center gap-1.5 text-[var(--color-text-secondary)]">
                          ...
                          <span>{locale === 'ka' ? `დაარსდა ${profile.foundedYear}` : `Est. ${profile.foundedYear}`}</span>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <section className="bg-[var(--color-bg-elevated)] rounded-2xl border border-[var(--color-border-subtle)] p-6 sm:p-8 shadow-sm">
                ...
                <div className="space-y-6">
                  {profile.categories.map((categoryKey) => {
                    const subcats = profile.subcategories?.filter(sub => {
                      for (const category of CATEGORIES) {
                        if (category.key === categoryKey) {
                          return category.subcategories.some(s => s.key === sub);
                        }
                      }
                      return false;
                    }) || [];
              ...
              <section className="bg-[var(--color-bg-elevated)] rounded-2xl border border-[var(--color-border-subtle)] p-6 sm:p-8 shadow-sm">
                ...
                {profile.teamMembers.length > 8 && (
                  <button className="w-full mt-4 py-3 text-sm font-semibold rounded-xl border border-[var(--color-border-subtle)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-tertiary)] transition-colors">
                    {locale === 'ka' ? `ყველას ნახვა (${profile.teamMembers.length})` : `View All (${profile.teamMembers.length})`}
                  </button>
                )}
              </section>
            )}
              ...
              <section className="bg-[var(--color-bg-elevated)] rounded-2xl border border-[var(--color-border-subtle)] p-6 sm:p-8 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {portfolioImages.length > 6 && (
                    <button className="text-sm font-semibold flex items-center gap-1 transition-colors hover:gap-2" style={{ color: ACCENT }}>
            ...
            <div className="lg:sticky lg:top-24 space-y-6">
                ...
                <div className="mt-6 space-y-3">
                  {profile.phone && (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-xs text-[var(--color-text-muted)]">{locale === 'ka' ? 'ტელეფონი' : 'Phone'}</p>
                        <p className="font-semibold text-[var(--color-text-primary)]">{profile.phone}</p>
                      </div>
                    </a>
                  )}
                  ...
                  {profile.email && (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-xs text-[var(--color-text-muted)]">{locale === 'ka' ? 'ელფოსტა' : 'Email'}</p>
                        <p className="font-semibold text-[var(--color-text-primary)] truncate">{profile.email}</p>
                      </div>
                    </a>
                  )}
                  ...
                  {profile.website && (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-xs text-[var(--color-text-muted)]">{locale === 'ka' ? 'ვებსაიტი' : 'Website'}</p>
                        <p className="font-semibold text-[var(--color-text-primary)] truncate">{profile.website}</p>
                      </div>
                    </a>
                  )}
                </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-blue-500/5 border border-blue-500/10">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-[var(--color-text-primary)]">
                          {locale === 'ka' ? 'ვერიფიცირებული კომპანია' : 'Verified Company'}
                        </p>
                        <p className="text-xs text-[var(--color-text-tertiary)]">
                          {locale === 'ka' ? 'კომპანია შემოწმებულია' : 'Company verified by Homico'}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {profile.isPremium && (
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-amber-500/5 border border-amber-500/10">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-[var(--color-text-primary)]">
                          {locale === 'ka' ? 'პრემიუმ კომპანია' : 'Premium Company'}
                        </p>
                        <p className="text-xs text-[var(--color-text-tertiary)]">
                          {locale === 'ka' ? 'სანდო და გამოცდილი' : 'Trusted & experienced'}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {profile.completedProjects && profile.completedProjects >= 10 && (
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-teal-500/5 border border-teal-500/10">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-[var(--color-text-primary)]">
                          {locale === 'ka' ? 'გამოცდილი კომპანია' : 'Experienced Company'}
                        </p>
                        <p className="text-xs text-[var(--color-text-tertiary)]">
                          {locale === 'ka'
              ...
              {profile.serviceAreas.length > 0 && (
                <div className="bg-[var(--color-bg-elevated)] rounded-2xl border border-[var(--color-border-subtle)] overflow-hidden shadow-sm">
                  {/* Map Illustration */}
                  <div className="aspect-[16/10] relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${ACCENT}10 0%, ${ACCENT}05 100%)` }}>
                    {/* Decorative Map Lines */}
                    <svg className="absolute inset-0 w-full h-full opacity-30" viewBox="0 0 200 125" preserveAspectRatio="none">
                      <path d="M0,60 Q50,40 100,60 T200,60" fill="none" stroke={ACCENT} strokeWidth="0.5" opacity="0.5"/>
                      <path d="M0,70 Q50,50 100,70 T200,70" fill="none" stroke={ACCENT} strokeWidth="0.5" opacity="0.3"/>
                      <circle cx="100" cy="60" r="20" fill={ACCENT} opacity="0.1"/>
                      <circle cx="100" cy="60" r="10" fill={ACCENT} opacity="0.15"/>
                    </svg>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <p className="text-sm font-bold" style={{ color: ACCENT }}>
                          {translateCity(profile.city || profile.serviceAreas[0])}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          >
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--color-bg-tertiary)] mb-6">
                {logoUrl ? (
                  <img src={getImageUrl(logoUrl)} alt="" className="w-14 h-14 rounded-xl object-cover" />
                ) : (
                  <div
                ...
                <div>
                  ...
                  <p className="text-sm text-[var(--color-text-secondary)]">{getCompanyTypeLabel()}</p>
                </div>
              </div>
