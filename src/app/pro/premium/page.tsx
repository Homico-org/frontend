...
import {
  Check, Crown, Sparkles, Star, Zap, Shield, TrendingUp, Award,
  Clock, MessageCircle, Eye, BadgeCheck, ChevronDown, Users,
  ArrowRight, Lock, CreditCard, RefreshCw, Headphones, CheckCircle2
} from 'lucide-react';
...
const stats = [
  ...
  { value: '500+', label: { en: 'Premium Professionals', ka: 'პრემიუმ პროფესიონალი' } },
...
function FAQItem({ question, answer, isOpen, onToggle }: { question: string; answer: string; isOpen: boolean; onToggle: () => void }) {
  return (
    <div className="border-b border-[var(--color-border-subtle)] last:border-0">
      ...
      >
        <span className="font-medium text-[var(--color-text-primary)] leading-relaxed group-hover:text-[#E07B4F] transition-colors">{question}</span>
        <div className={`w-8 h-8 rounded-full bg-[var(--color-bg-tertiary)] flex items-center justify-center flex-shrink-0 transition-all duration-300 ${isOpen ? 'bg-[#E07B4F]/10 rotate-180' : ''}`}>
          <ChevronDown className={`w-4 h-4 transition-colors ${isOpen ? 'text-[#E07B4F]' : 'text-[var(--color-text-tertiary)]'}`} />
        </div>
      </button>
    ...
    <div className="bg-[var(--color-bg-elevated)] rounded-2xl p-6 border border-[var(--color-border-subtle)] shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      <div className="mt-4 pt-4 border-t border-[var(--color-border-subtle)]">
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
          testimonial.tier === 'elite' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
          testimonial.tier === 'pro' ? 'bg-[#E07B4F]/10 text-[#E07B4F]' :
          'bg-[#4A9B9B]/10 text-[#4A9B9B]'
        }`}>
          {testimonial.tier === 'elite' ? 'Elite' : testimonial.tier === 'pro' ? 'Pro' : 'Premium'} {locale === 'ka' ? 'მომხმარებელი' : 'Member'}
        </span>
      </div>
    </div>
  );
...
export default function PremiumPlansPage() {
  ...
  const faqs = [
    ...
    {
      q: { en: 'Is my payment information secure?', ka: 'დაცულია ჩემი გადახდის ინფორმაცია?' },
      a: { en: 'Absolutely. We use bank-level SSL encryption and never store your full card details. All payments are processed through certified payment providers.', ka: 'აბსოლუტურად. ვიყენებთ საბანკო დონის SSL დაშიფვრას და არასდროს ვინახავთ თქვენს სრულ ბარათის მონაცემებს.' }
    },
  ];
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 relative">
            ...
            <div className="flex justify-center mb-8">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--color-bg-elevated)] border border-[var(--color-border-subtle)] shadow-sm">
                <div className="flex -space-x-2">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="w-7 h-7 rounded-full bg-gradient-to-br from-[#4A9B9B] to-[#3D8585] border-2 border-[var(--color-bg-elevated)] flex items-center justify-center text-white text-xs font-medium">
                      {String.fromCharCode(65 + i)}
                    </div>
                  ))}
                </div>
                <span className="text-sm text-[var(--color-text-secondary)] ml-1">
                  <span className="font-semibold text-[var(--color-text-primary)]">500+</span> {locale === 'ka' ? 'პროფესიონალი' : 'professionals trust us'}
                </span>
              </div>
            </div>
                  ...
                  <>
                    ...
                    <span className="relative inline-block">
                      ...
                      <svg className="absolute -bottom-1 left-0 w-full h-3" viewBox="0 0 200 12" fill="none" preserveAspectRatio="none">
                        <path d="M2 8C50 4 150 4 198 8" stroke="#E07B4F" strokeWidth="3" strokeLinecap="round" strokeOpacity="0.3"/>
                      </svg>
                    </span>
                    <br />პრემიუმ გეგმით
                  </>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 max-w-3xl mx-auto mb-16">
              {stats.map((stat, i) => (
                ...
                >
                  <div className="text-2xl sm:text-3xl font-bold text-[#E07B4F] mb-1">{stat.value}</div>
                  <div className="text-xs sm:text-sm text-[var(--color-text-tertiary)]">{stat.label[locale === 'ka' ? 'ka' : 'en']}</div>
                </div>
              ))}
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-6 sm:p-8 pt-8">
                      ...
                      <div className="mb-6 pb-6 border-b border-[var(--color-border-subtle)]">
                        ...
                        {billingPeriod === 'yearly' && (
                          <p className="mt-2 text-sm text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                            ...
                            {locale === 'ka' ? `დაზოგე ${tier.currency}${monthlyTotal - yearlyPrice}` : `Save ${tier.currency}${monthlyTotal - yearlyPrice}/year`}
                          </p>
                        )}
                      </div>
                          ...
                          return (
                            <li key={i} className={`flex items-start gap-3 ${feature.included ? 'opacity-60' : ''}`}>
                              <div
                                ...
                                style={{
                                  backgroundColor: feature.included ? 'var(--color-bg-tertiary)' : `${tier.accentColor}15`,
                                }}
                              >
                                {feature.included ? (
                      ...
                      >
                        {isCurrentPlan ? (
                          <><Check className="w-4 h-4" /> {locale === 'ka' ? 'მიმდინარე გეგმა' : 'Current Plan'}</>
                        ) : (
                          <>{locale === 'ka' ? 'აირჩიე გეგმა' : 'Get Started'} <ArrowRight className="w-4 h-4" /></>
                        )}
                      </button>
                    </div>
                  </div>
              ...
              {[
                {
                  ...
                  desc: { en: 'Full refund, no questions', ka: 'სრული თანხის დაბრუნება' },
                  color: '#E07B4F'
                },
                {
                  icon: Lock,
                  title: { en: 'SSL Encrypted', ka: 'SSL დაშიფრული' },
                  desc: { en: 'Bank-level security', ka: 'საბანკო დონის დაცვა' },
                  color: '#4A9B9B'
                },
                {
                  icon: CreditCard,
                  title: { en: 'Secure Payments', ka: 'უსაფრთხო გადახდა' },
                  desc: { en: 'PCI DSS compliant', ka: 'PCI DSS სერტიფიცირებული' },
                  color: '#6366F1'
                },
                {
                  icon: RefreshCw,
                  title: { en: 'Cancel Anytime', ka: 'გაუქმება ნებისმიერ დროს' },
                  desc: { en: 'No lock-in contracts', ka: 'ხელშეკრულების გარეშე' },
                  color: '#10B981'
                },
              ].map((item, i) => (
                <div
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-12">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 text-sm font-medium mb-4">
                ...
                {locale === 'ka' ? 'კლიენტების მოწონება' : 'Loved by Professionals'}
              </div>
              <h2 className="text-3xl sm:text-4xl font-bold text-[var(--color-text-primary)] mb-4">
                {locale === 'ka' ? 'რას ამბობენ ჩვენი მომხმარებლები' : 'What Our Members Say'}
              </h2>
              <p className="text-[var(--color-text-secondary)] max-w-2xl mx-auto">
                {locale === 'ka'
          ...
          <div className="max-w-3xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-12">
              ...
              <p className="text-[var(--color-text-secondary)]">
                {locale === 'ka' ? 'პასუხები გავრცელებულ კითხვებზე' : 'Quick answers to common questions'}
              </p>
            </div>
            ...
            <div className="mt-8 text-center">
              <p className="text-[var(--color-text-secondary)] mb-4">
                {locale === 'ka' ? 'კიდევ გაქვს კითხვები?' : 'Still have questions?'}
              </p>
              <a
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#E07B4F] to-[#D26B3F] p-8 sm:p-12 text-center text-white">
              ...
              <div className="relative">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur-sm text-white/90 text-sm font-medium mb-6">
                  ...
                  {locale === 'ka' ? 'გაზარდე შესაძლებლობები' : 'Level Up Today'}
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                  ...
                  <div className="flex items-center gap-2 text-white/80 text-sm">
                    <Shield className="w-4 h-4" />
                    {locale === 'ka' ? '7 დღის თანხის დაბრუნების გარანტია' : '7-day money-back guarantee'}
                  </div>
                </div>
              </div>
            </div>
