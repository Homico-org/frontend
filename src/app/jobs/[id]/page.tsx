...
const statusMap: Record<string, { label: string; color: string }> = {
  'open': { label: 'აქტიური', color: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' },
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--color-bg-primary)' }}>
      ...
      {success && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-fade-in">
          <div className="px-5 py-3 rounded-xl shadow-lg flex items-center gap-3" style={{ backgroundColor: 'var(--color-bg-elevated)', border: '1px solid var(--color-border)' }}>
            <div className="w-5 h-5 rounded-full bg-emerald-500 flex items-center justify-center flex-shrink-0">
              <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <span className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>{success}</span>
            <button onClick={() => setSuccess('')} className="ml-2 p-1 rounded-lg transition-colors" style={{ color: 'var(--color-text-muted)' }}>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-10">
            ...
            <div className="lg:col-span-2 space-y-6 sm:space-y-10">
              ...
              {job.description && (
                <section>
                  <h2 className="text-xs uppercase tracking-wider font-medium mb-4" style={{ color: 'var(--color-text-muted)' }}>აღწერა</h2>
                  <p className="text-[15px] leading-relaxed whitespace-pre-wrap" style={{ color: 'var(--color-text-secondary)' }}>
                    {job.description}
                  </p>
                </section>
              )}
              ...
              {job.workTypes && job.workTypes.length > 0 && (
                <section>
                  <h2 className="text-xs uppercase tracking-wider font-medium mb-4" style={{ color: 'var(--color-text-muted)' }}>სამუშაოს ტიპები</h2>
              ...
              {(job.designStyle || (job.roomsToDesign && job.roomsToDesign.length > 0)) && (
                <section>
                  <h2 className="text-xs uppercase tracking-wider font-medium mb-4" style={{ color: 'var(--color-text-muted)' }}>დიზაინის დეტალები</h2>
                  <div className="space-y-4">
                    ...
                    {job.roomsToDesign && job.roomsToDesign.length > 0 && (
                      <div>
                        <span className="text-sm block mb-2" style={{ color: 'var(--color-text-tertiary)' }}>ოთახები:</span>
                        <div className="flex flex-wrap gap-2">
              ...
              {(job.furnitureIncluded || job.visualizationNeeded || job.materialsProvided !== undefined || job.occupiedDuringWork) && (
                <section>
                  <h2 className="text-xs uppercase tracking-wider font-medium mb-4" style={{ color: 'var(--color-text-muted)' }}>დამატებითი ინფორმაცია</h2>
                  <div className="flex flex-wrap gap-2">
                    {job.furnitureIncluded && (
                      <span className="text-sm px-3 py-1.5 rounded-lg flex items-center gap-2" style={{ backgroundColor: 'var(--color-bg-tertiary)', color: 'var(--color-text-secondary)' }}>
                    ...
                    {job.occupiedDuringWork && (
                      <span className="text-sm px-3 py-1.5 rounded-lg flex items-center gap-2" style={{ backgroundColor: 'var(--color-highlight-soft)', color: 'var(--color-highlight)' }}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {job.skills && job.skills.length > 0 && (
                <section>
                  <h2 className="text-xs uppercase tracking-wider font-medium mb-4" style={{ color: 'var(--color-text-muted)' }}>უნარები</h2>
                  <div className="flex flex-wrap gap-2">
              ...
              {job.references && job.references.length > 0 && (
                <section>
                  <h2 className="text-xs uppercase tracking-wider font-medium mb-4" style={{ color: 'var(--color-text-muted)' }}>რეფერენსები</h2>
                  <div className="grid gap-2">
                    {job.references.map((ref, idx) => (
                      <a
                        ...
                        className="flex items-center gap-3 p-3 rounded-xl transition-all duration-200 group hover:scale-[1.01]"
                      ...
                      >
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'var(--color-bg-tertiary)' }}>
                          {ref.type === 'pinterest' ? (
                            ...
                            <svg className="w-5 h-5" style={{ color: 'var(--color-text-tertiary)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate group-hover:text-[var(--color-accent)] transition-colors" style={{ color: 'var(--color-text-primary)' }}>
                            {ref.title || new URL(ref.url).hostname}
                          </p>
                          <p className="text-xs truncate" style={{ color: 'var(--color-text-muted)' }}>{ref.url}</p>
                        </div>
                        <svg className="w-4 h-4 flex-shrink-0 group-hover:translate-x-0.5 transition-transform" style={{ color: 'var(--color-text-muted)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {user?.role === 'pro' && job.status === 'open' && !myProposal && (
                <section className="pt-8" style={{ borderTop: '1px solid var(--color-border-subtle)' }}>
                  {!showProposalForm ? (
                    <button
                      onClick={() => setShowProposalForm(true)}
                      className="w-full py-4 text-sm font-medium rounded-xl transition-all duration-200 hover:scale-[1.01] active:scale-[0.99]"
                      style={{ backgroundColor: 'var(--color-accent)', color: 'white' }}
                    >
                      წინადადების გაგზავნა
                    </button>
                  ) : (
                    <form onSubmit={handleSubmitProposal} className="p-6 rounded-2xl space-y-6 animate-fade-in" style={{ backgroundColor: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)' }}>
                      ...
                      <div className="flex gap-3 pt-2">
                        <button
                          ...
                          className="flex-1 py-3.5 text-sm font-medium rounded-xl transition-all duration-200"
                        ...
                        <button
                          ...
                          className="flex-1 py-3.5 text-sm font-medium rounded-xl transition-all duration-200 disabled:opacity-50 hover:opacity-90"
                          style={{ backgroundColor: 'var(--color-accent)', color: 'white' }}
                        >
                          {isSubmitting ? (
