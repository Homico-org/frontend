...
const categoryLabels: Record<string, { en: string; ka: string }> = {
...
export default function JobDetailPage() {
  ...
  const getCategoryLabel = (cat: string) => {
    ...
    return label ? label[locale as 'en' | 'ka'] : cat.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  };
        ...
        <div className="max-w-3xl mx-auto px-6 py-8">
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              {isHired && (
                <span className="inline-flex items-center gap-1.5 text-[11px] font-medium" style={{ color: ACCENT_COLOR }}>
                      ...
                      <div className="w-56 h-40 bg-neutral-800 flex items-center justify-center">
                        {media.thumbnail ? (
                          ...
                          <div className="w-12 h-12 rounded-full bg-white/90 flex items-center justify-center">
                            <svg className="w-5 h-5 ml-0.5" style={{ color: ACCENT_COLOR }} fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
              ...
              {job.deadline && (
                <div className="p-4 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800">
                  ...
                  <p className="text-sm font-semibold text-neutral-900 dark:text-white">
                    {new Date(job.deadline).toLocaleDateString(locale === 'ka' ? 'ka-GE' : 'en-US', { month: 'short', day: 'numeric' })}
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="space-y-2">
                {job.furnitureIncluded && (
                  <div className="flex items-center gap-2 text-neutral-600 dark:text-neutral-300">
                    ...
                    <span className="text-sm">{locale === 'ka' ? 'ავეჯის შერჩევა' : 'Furniture Selection'}</span>
                  </div>
                )}
                {job.visualizationNeeded && (
                  <div className="flex items-center gap-2 text-neutral-600 dark:text-neutral-300">
                    <Check className="w-4 h-4 text-green-500" />
                    <span className="text-sm">{locale === 'ka' ? '3D ვიზუალიზაცია' : '3D Visualization'}</span>
                  </div>
                )}
                {job.materialsProvided && (
                  <div className="flex items-center gap-2 text-neutral-600 dark:text-neutral-300">
                    <Check className="w-4 h-4 text-green-500" />
                    <span className="text-sm">{locale === 'ka' ? 'მასალები უზრუნველყოფილია' : 'Materials Provided'}</span>
                  </div>
                )}
                {job.occupiedDuringWork && (
                  <div className="flex items-center gap-2 text-neutral-600 dark:text-neutral-300">
                    <Check className="w-4 h-4 text-green-500" />
                    <span className="text-sm">{locale === 'ka' ? 'დაკავებული სამუშაოს დროს' : 'Occupied During Work'}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 mb-8">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className={`text-xs px-2 py-1 rounded-md font-medium ${
                  myProposal.status === 'pending' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                  myProposal.status === 'accepted' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
              ...
              <p className="text-sm text-neutral-600 dark:text-neutral-300 mb-3">{myProposal.coverLetter}</p>
                ...
                <div className="flex gap-6 text-sm">
                  ...
                  {myProposal.estimatedDuration && (
                    <span className="text-neutral-500">
                      {myProposal.estimatedDuration} {
                        myProposal.estimatedDurationUnit === 'days' ? (locale === 'ka' ? 'დღე' : 'days') :
                        myProposal.estimatedDurationUnit === 'weeks' ? (locale === 'ka' ? 'კვირა' : 'weeks') :
      ...
      {success && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-slide-down">
          <div className="flex items-center gap-3 px-5 py-3 rounded-xl bg-green-500 text-white shadow-lg">
            ...
            <button onClick={() => setSuccess('')} className="p-1 hover:bg-white/20 rounded-full transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
